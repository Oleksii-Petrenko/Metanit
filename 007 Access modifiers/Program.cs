...
namespace _007_Access_modifiers
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            state2.defaultMethod(); //Ошибка, получить доступ нельзя

            state2.privateMethod(); // Ошибка, получить доступ нельзя

            state2.protectedPrivateMethod(); // Ошибка, получить доступ нельзя

            state2.protectedMethod(); // Ошибка, получить доступ нельзя

            state2.internalMethod();    // норм

            state2.protectedInternalMethod();  // норм

            state2.publicMethod();      // норм
